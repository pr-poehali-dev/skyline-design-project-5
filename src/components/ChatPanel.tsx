...
const QUICK_CHIPS = ["Кто ты?", "Расскажи об изобретении", "Как работает бандаж?"]

const RESPONSES: Record<string, string> = {
  "Кто ты?": "Привет! Я Маша, мне 14 лет. Живу в Заполярном, занимаюсь плаванием 5 лет и изобрела умный бандаж «Судорогам — Стоп»!",
  "Расскажи об изобретении":
    "«Судорогам — Стоп» — ортопедический бандаж с тремя режимами: электростимуляция, прогревание и компрессия. Управляется через приложение!",
  "Как работает бандаж?":
    "При судороге включаешь «Экстренную помощь» — устройство подаёт слабый электроимпульс, который снимает спазм. Затем прогревание улучшает кровоток, а компрессия стабилизирует мышцу.",
}

const ACTION_RESPONSES: Record<string, { response: string; action: string }> = {
  "открой галерею": { response: "Открываю галерею!", action: "art" },
  "покажи галерею": { response: "Открываю раздел с фото!", action: "art" },
  "открой резюме": { response: "Открываю информацию о проекте!", action: "resume" },
  "покажи резюме": { response: "Вот описание изобретения!", action: "resume" },
  "открой обо мне": { response: "Открываю раздел обо мне!", action: "about" },
  "покажи обо мне": { response: "Расскажу о Маше!", action: "about" },
  "открой статьи": { response: "Открываю материалы о проекте!", action: "writings" },
  "покажи статьи": { response: "Вот статьи о разработке!", action: "writings" },
  "режимы": { response: "Три режима: 1) Электростимуляция — снимает судорогу, 2) Прогревание — улучшает кровоток, 3) Компрессия — стабилизация в восстановлении.", action: "resume" },
}
  ...
  const handleInputSubmit = (e: React.FormEvent) => {
    ...
    if (actionMatch) {
      ...
      const defaultResponse =
        "Интересный вопрос! Напиши «покажи обо мне», «открой резюме» или «открой галерею» — и я покажу нужный раздел!"
